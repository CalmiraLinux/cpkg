...
* `BEF_DEPS` - те пакеты, которые должны быть установлены первее
...
| `BEF_DEPS` | необязательные зависимости, которые должны быть установлены ДО установки исходного пакета. | `Установлены первее:` |
...
В репозитории ПО, зачастую, не самая последняя версия `cpkg`. Для того, чтобы установить самую последнюю, нужно скачать её с этого репа и установить. Принцип действий таков:
* Скачивание пакета
...
wget https://github.com/Linuxoid85/cpkg/releases/download/1.0.pa4/cpkg.txz
